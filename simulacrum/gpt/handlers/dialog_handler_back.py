...
class DialogueHandler(BaseHandler):
    ...
    def generate_dialogue_turn(
        ...
        # 統整的對話生成指引
        prompt += f"\n請扮演{speaker.name}，產生一句話回應{listener.name}。\n"
        
        # 統一的指引格式
        prompt += "\n對話要求：\n"
        
        # 主題與意圖指引
        if current_topic and speaker_intent:
            prompt += f"- 自然回應對方的話，圍繞主題「{current_topic}」\n"
            prompt += f"- 體現意圖：{speaker_intent}\n"
            prompt += f"- 避免重複說過的內容，但不要過度思考\n"
        
        # 統一的規則格式
        prompt += "- 僅輸出一句話，無旁白或動作描述\n"
        prompt += "- 像朋友聊天一樣自然回應\n"
        prompt += "- 不要像在演戲、講故事或做簡報\n"
        prompt += "- 避免過度解釋或自我分析\n"
        prompt += "- 簡單口語，不要刻意或做作\n"
